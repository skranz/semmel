...
show.new.ui = function(start.pane = "home",...) {
  ui = navlistPanel(id="mainPanel", selected=start.pane,
    tabPanel("Semmel",value="home", new.home.ui()),
    ...
    tags$table(style="width: 100%",
      tags$tr(
        tags$td(style="width: 50%", valign="top",
          textAreaInput("topicsInput","Topics",value = tat$topic.text, cols=80, rows=10, resize="both")
        ),
...
new.step2.ui = function(...,tat=app$tat, app=getApp(), glob=app$glob) {
  ...
  ui = tagList(div(id="app_create_2_div",
    ...
    selectInput("topn","How do students rank topics?", list("Rank all topics (recommended for feasible matching)"=NA_integer_,"Choose Top-3"=3,"Choose Top-5"=5,"Choose Top-10"=10, "Rank arbitrary many"=-1)),
...
parse.topic.text = function(topic.text, multi.line = FALSE, def_slots=1) {
  ...
  topics = topics.md2html(txt)
  list(topics=topics, slots=slots)
}
...
verify.tat = function(tat) {
  ...
  if (!isTRUE(tat$agree)) {
    return(list(ok=FALSE, msg="To use Semmel, please check the checkbox above, to allow us to use and share anonymized data for research purposes.

<br>Background: Semmel was generated at the Department of Mathematics and Economics at Ulm University. On the one hand, we just wanted to provide a simple tool to solve the common problem of assigning seminar topics. On the other hand, we also want to use anonymized data to further our understanding, how allocation mechanisms are used in practice. For the sake of open and reproducible science, we would also have the permission to share anonomymized data. There is only academic interest in the data."))
  }
